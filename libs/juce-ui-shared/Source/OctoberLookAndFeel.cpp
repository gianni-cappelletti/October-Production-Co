                                          ...
                                          float rotaryEndAngle, juce::Slider& slider)
{
  ...
  else
  {
    ...
    const int numTicks = static_cast<int>(slider.getProperties().getWithDefault("numTicks", 11));
    const float tickStart = static_cast<float>(
        slider.getProperties().getWithDefault("tickStartAngle", rotaryStartAngle));
    const float tickEnd =
        static_cast<float>(slider.getProperties().getWithDefault("tickEndAngle", rotaryEndAngle));
    const float tickInner = outerR + 3.0f;
    const float tickOuter = outerR + 9.0f;
    g.setColour(juce::Colour(0xff888888));
    for (int i = 0; i < numTicks; ++i)
    {
      float tickAngle = tickStart + (float)i / (float)(numTicks - 1) * (tickEnd - tickStart);
